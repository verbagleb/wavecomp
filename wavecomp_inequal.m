...
decimation = [6 3 2]; %коэффициенты децимации для каждой полосы
length = 6;
      %условия для на произодные прямых фильтров в заданных точках
...
H=sym('H',[N_bands,length]);
for n=1:N_bands
    for k=1:length
        H(n,k) = sum(hz(n,mod(k-1,decimation(n))+1:decimation(n):end));
    end
end
H=H.';
for n=1:N_bands
...
N_cons_cond=length-N_bands;
fprintf('Условий на совместимость системы: %d\n', N_cons_cond);
DH=repmat(H(1:3,:).',1,1,N_bands);
lambda=sym('l',[N_bands,1]);
eq_cons=sym('eq_cons',[N_cons_cond,1]);
for k=1:N_cons_cond
    for i=1:N_bands
        DH(:,i,i)=H(4,:).';
        lambda(i)=det(DH(:,:,i));
    end
    eq_cons(k) = sum(lambda)==det0;
end
